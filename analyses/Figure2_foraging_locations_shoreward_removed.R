...
################################################################################
# Step 6 - build kelp forest, margin, edge, land, and benthic site buffer layers
...
################################################################################
# Step 8 - identify and drop landward white-zone dives
# These are outside-canopy points located between kelp canopy and land.
# Logic: for each dive, compare distance to land for the dive itself vs.
# distance to land for the nearest point on the kelp edge.
# If the dive is closer to land than its nearest kelp edge point, it is landward.

nearest_edge_lines <- st_nearest_points(
  st_geometry(focal_dives_raw),
  kelp_edge_geom
)
...
focal_dives_screened <- focal_dives_raw %>%
  mutate(
...
message(
  "Dropped ",
  sum(focal_dives_screened$landward_white_zone, na.rm = TRUE),
  " landward white-zone dives out of ",
  nrow(focal_dives_screened),
  " successful urchin dives."
)
...
ggsave(
  here("figures", "Fig2_white_zone_dropped_points_check.png"),
  p_white_zone_check,
  width = 7,
  height = 6,
  dpi = 600,
...
################################################################################
# Step 13 - calculate distance of retained successful urchin dives to nearest kelp patch edge
...
################################################################################
# Step 14 - bin retained successful urchin dives by 20 m distance bins
...
################################################################################
# Step 15 - Panel A: map of retained successful urchin foraging dives
...
################################################################################
# Step 16 - Panel B: barplot of focal bouts by canopy zone
...
################################################################################
# Step 17 - Panel C: retained successful urchin dives relative to nearest kelp patch
