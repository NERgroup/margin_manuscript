...
base_theme <- theme(
  ...
  strip.text = element_text(size = 10, face = "bold", hjust = 0, color = "black")
)
...
################################################################################
#Step 8 - Panels B-E: faceted margin-transition figure
...
p_faceted_margin <- ggplot(
  ...
  facet_wrap(
    ...
    labeller = labeller(
      metric = c(
        "Macroalgal density" =
          "B. Macroalgal density [log(x + 1)\n(no. stipe per 10m²)]",
        "Behavior" =
          "C. Proportion of concealed sea urchins",
        "Urchin density" =
          "D. Purple urchin density\n(no. indiv. per 10m²)",
        "Gonad index" =
          "E. Gonad index"
      )
    )
...
focal_xy <- st_coordinates(focal_pt)
